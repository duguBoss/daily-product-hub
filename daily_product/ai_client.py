        ...
        for provider, model_name, api_key, call_func in self.candidates:
            ...
            try:
                ...
                if not result or not result.strip():
                    print(f"   ⚠️ {provider}/{model_name} 返回空内容，切换到下一个模型...")
                    attempts += 1
                    continue
...
请分析以下网页内容，提取 {NEWS_COUNT} 条【科技数码产品】相关新闻。

=== 内容类型 ===
- 手机、电脑、平板等数码产品发布/评测/价格变动
- 显卡、CPU、芯片等硬件产品
- 智能家居、穿戴设备、汽车科技
- 科技行业重要动态、公司财报/发布会
...
优先选择【今日新闻】（带时:分格式的），如果不足 {NEWS_COUNT} 条，再补充最近1-2天的重要新闻。

=== 选择标准 ===
- 必须有具体产品名称或型号
- 优先热门品牌（苹果、三星、华为、小米、OPPO、vivo等）
- 新闻链接通常是 https://www.ithome.com/... 或 https://news.mydrivers.com/... 格式

=== 输出格式（必须严格遵循） ===
只返回 JSON 数组，不要任何其他文字：
[{{"title": "华为P70系列发布，搭载麒麟9000S芯片", "url": "https://www.ithome.com/0/xxx/xxx.htm"}}]
